...
const CardDetail = ({ course, setModalDetail }) => {
		...
		<>
			{router.pathname !== '/my-courses/learning' && (
				<div
					...
					<h6 className={styles.title}>{course != undefined && course.title}</h6>
					<div >{date !== undefined && date.map(item => (
						<div key={item.id}>{item.mes} {day}</div>
					))}</div>
					<span className={styles.description}>{course != undefined && course.description}</span>
					<div className={styles.what_learnt}>
						{course != undefined && course.what_learnt.slice(0, 3).map(item => (
							<>
								<div className={styles.what_learnt_list}> &#10003; {item.title}</div>
							</>
						))}
					</div>
